...
public class CheckoutController implements Initializable {
 ...
 /* void checkIfIssued(){
        ...
        String qu = "SELECT * FROM room WHERE roomid = '" + roomid + "'";
        ResultSet rsChecker = databaseHandler.execQuery(qu);
        try {
            if(rsChecker.getString("availability").equalsIgnoreCase("unavailability") ){
                System.out.println(rsChecker.getString("availability"));
                checker = true;                
            }
    ...
    */
    ...
    private void proceedOP(ActionEvent event) {
        if(lblStatus.getText().equalsIgnoreCase("unavailable")){
            checker = true;
        }
        if(btnPaid.isSelected()){
            if(customerReady && roomReady && checker){
                ...
                alert.setTitle("Confirm Check Out Operation");
                ...
                if(response.get() == ButtonType.OK){
                      ...
                      //Update room details
                      String str3 = "UPDATE room SET availability = 'Available' WHERE roomid = '" + lblID.getText() + "'";

                      System.out.println(str3);
                     if(databaseHandler.execAction(str3)) {
                            customerReady = false; roomReady = false;
                            Alert alert1 = new Alert(Alert.AlertType.INFORMATION);
                            alert1.setTitle("Successful");
                            alert1.setHeaderText(null);
                            alert1.setContentText("Customer Checked Out !" );
                            alert1.showAndWait();
                        }
                        else {
